...
export default function RomancePage() {
  ...
  // Get romance videos from the video library
  const romanceVideos = getVideosByCategory('romance');
  ...
  return (
    <div className="min-h-screen bg-white md:bg-gray-900 text-gray-900 md:text-white">
          ...
          <div className="flex items-center justify-between relative">
            ...
            <div className="hidden md:flex flex-1 max-w-2xl mx-8">
              <div className="relative w-full">
                <Input
                  type="text"
                  placeholder="Search romance videos..."
            ...
            {categories.map((category) => (
              <Link
                ...
                className={`text-gray-300 hover:text-white whitespace-nowrap px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors text-caption font-medium btn-professional focus-professional ${
                  category.name === "Romance" ? "bg-red-600 text-white" : ""
                }`}
              >
                {category.name}
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 md:text-white mb-2">Romance</h1>
          <p className="text-gray-600 md:text-gray-400">Fall in love with heartwarming stories, passionate tales, and romantic adventures</p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <main className="flex-1">
            {/* Reklam Alanı */}
            <div className="mb-8 p-6 bg-gradient-to-r from-pink-500 to-red-500 rounded-lg text-white text-center">
              <div className="flex items-center justify-center space-x-4">
                <div className="text-lg font-bold">💕 ROMANCE SPONSORLAR</div>
                <div className="text-sm opacity-90">Romantik İçerikler Premium Reklam</div>
              </div>
              <div className="mt-2 text-xs opacity-75">Romance kategorisi reklam entegrasyonu alanı</div>
            </div>

            <div className="video-grid">
              {romanceVideos.map((video) => (
                <VideoCard
                  key={video.id}
                  video={video}
                  showCategory={false}
            ...
            <div className="flex justify-center items-center space-x-4 mt-12 pt-8 border-t border-gray-200 md:border-gray-800">
              <span className="text-gray-600 md:text-gray-400 text-caption">Showing {romanceVideos.length} romance videos, Page 1</span>
          ...
          <aside className="hidden md:block w-72 bg-black rounded-xl p-6 h-fit shadow-professional-lg">
            <h2 className="text-white text-title mb-6">Romance Types</h2>
            <div className="space-y-3">
              {["Contemporary", "Historical", "Romantic Comedy", "Workplace Romance", "Holiday Romance", "International", "Second Chance", "Friends to Lovers", "Enemies to Lovers", "Long Distance", "Musical Romance", "Fantasy Romance"].map((genre) => (
                <a
                  key={genre}
                  href="#"
            ...
            {/* Featured Romance Videos */}
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Top Romance</h3>
              <div className="space-y-4">
                {romanceVideos.slice(0, 4).map((video) => (
