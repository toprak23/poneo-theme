...
export default function TVShowsPage() {
  ...
  // Get TV shows from the video library
  const tvShows = getVideosByCategory('tv-shows');
  ...
  return (
    <div className="min-h-screen bg-white md:bg-gray-900 text-gray-900 md:text-white">
            ...
            {categories.map((category) => (
              <Link
                ...
                className={`text-gray-300 hover:text-white whitespace-nowrap px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors text-caption font-medium btn-professional focus-professional ${
                  category.name === "TV Shows" ? "bg-red-600 text-white" : ""
                }`}
              >
                {category.name}
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 md:text-white mb-2">TV Shows</h1>
          <p className="text-gray-600 md:text-gray-400">Binge-worthy series, captivating storylines, and episodic entertainment</p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <main className="flex-1">
            {/* Reklam Alanı */}
            <div className="mb-8 p-6 bg-gradient-to-r from-indigo-600 to-purple-600 rounded-lg text-white text-center">
              <div className="flex items-center justify-center space-x-4">
                <div className="text-lg font-bold">📺 TV SHOWS SPONSORLAR</div>
                <div className="text-sm opacity-90">Dizi İçerikleri Premium Reklam</div>
              </div>
              <div className="mt-2 text-xs opacity-75">TV Shows kategorisi reklam entegrasyonu alanı</div>
            </div>
            ...
            <div className="flex justify-center items-center space-x-4 mt-12 pt-8 border-t border-gray-200 md:border-gray-800">
              <span className="text-gray-600 md:text-gray-400 text-caption">Showing {tvShows.length} TV shows, Page 1</span>
          ...
          <aside className="hidden md:block w-72 bg-black rounded-xl p-6 h-fit shadow-professional-lg">
            <h2 className="text-white text-title mb-6">TV Show Genres</h2>
            <div className="space-y-3">
              {["Drama Series", "Comedy Series", "Crime Drama", "Sci-Fi Series", "Medical Drama", "Legal Drama", "Reality TV", "Documentary Series", "Animated Series", "Mini-Series", "Talk Shows", "Game Shows"].map((genre) => (
                <a
                  key={genre}
                  href="#"
            ...
            {/* Featured TV Shows */}
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Top TV Shows</h3>
              <div className="space-y-4">
                {tvShows.slice(0, 4).map((video) => (
            ...
            {/* TV Show Archive Dropdown */}
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Season Status</h3>
              <select className="w-full bg-gray-800 border border-gray-700 text-white text-body rounded-lg px-4 py-3 focus-professional">
                ...
                <option>Completed Series</option>
                <option>New Seasons</option>
                <option>Returning Soon</option>
