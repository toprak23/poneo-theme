...
export default function ActionPage() {
  ...
  // Get action videos from the video library
  const actionVideos = getVideosByCategory('action');
  ...
  return (
    <div className="min-h-screen bg-white md:bg-gray-900 text-gray-900 md:text-white">
          ...
          <div className="flex items-center justify-between relative">
            ...
            <div className="hidden md:flex flex-1 max-w-2xl mx-8">
              <div className="relative w-full">
                <Input
                  type="text"
                  placeholder="Search action videos..."
            ...
            {categories.map((category) => (
              <Link
                ...
                className={`text-gray-300 hover:text-white whitespace-nowrap px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors text-caption font-medium btn-professional focus-professional ${
                  category.name === "Action" ? "bg-red-600 text-white" : ""
                }`}
              >
                {category.name}
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 md:text-white mb-2">Action</h1>
          <p className="text-gray-600 md:text-gray-400">High-octane thrills, explosive stunts, and adrenaline-pumping adventures</p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <main className="flex-1">
            {/* Reklam Alanı */}
            <div className="mb-8 p-6 bg-gradient-to-r from-red-600 to-orange-600 rounded-lg text-white text-center">
              <div className="flex items-center justify-center space-x-4">
                <div className="text-lg font-bold">🎬 ACTION SPONSORLAR</div>
                <div className="text-sm opacity-90">Aksiyon Filmler Premium İçerik</div>
              </div>
              <div className="mt-2 text-xs opacity-75">Action kategorisi reklam entegrasyonu alanı</div>
            </div>
            ...
            <div className="flex justify-center items-center space-x-4 mt-12 pt-8 border-t border-gray-200 md:border-gray-800">
              <span className="text-gray-600 md:text-gray-400 text-caption">Showing {actionVideos.length} action videos, Page 1</span>
          ...
          <aside className="hidden md:block w-72 bg-black rounded-xl p-6 h-fit shadow-professional-lg">
            <h2 className="text-white text-title mb-6">Action Subgenres</h2>
            <div className="space-y-3">
              {["Superhero", "Martial Arts", "Military", "Spy/Espionage", "Chase", "Heist", "Adventure", "Disaster", "War", "Crime Action", "Sci-Fi Action", "Fantasy Action"].map((genre) => (
                <a
                  key={genre}
                  href="#"
            ...
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Top Action</h3>
              <div className="space-y-4">
                {actionVideos.slice(0, 4).map((video) => (
