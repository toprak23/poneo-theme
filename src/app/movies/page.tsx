...
export default function MoviesPage() {
  ...
  const sidebarCategories = [
    "Popular", "Trending", "New Releases", "Top Rated", "Comedy", "Action",
    "Drama", "Horror", "Sci-Fi", "Romance", "Anime", "Documentary", "Thriller",
    "Family", "Adventure", "Fantasy", "Crime", "War", "Western", "Musical"
  ];

  // Get movies from the video library
  const movies = getVideosByCategory('movies');
  ...
  return (
    <div className="min-h-screen bg-white md:bg-gray-900 text-gray-900 md:text-white">
            ...
            {categories.map((category) => (
              <Link
                ...
                className={`text-gray-300 hover:text-white whitespace-nowrap px-4 py-2 rounded-lg hover:bg-gray-800 transition-colors text-caption font-medium btn-professional focus-professional ${
                  category.name === "Movies" ? "bg-red-600 text-white" : ""
                }`}
              >
                {category.name}
              </Link>
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        {/* Page Title */}
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-gray-900 md:text-white mb-2">Movies</h1>
          <p className="text-gray-600 md:text-gray-400">Discover our collection of blockbuster movies and cinematic masterpieces</p>
        </div>
        ...
        <div className="flex gap-8">
          ...
          <main className="flex-1">
            {/* Reklam Alanı */}
            <div className="mb-8 p-6 bg-gradient-to-r from-blue-600 to-cyan-600 rounded-lg text-white text-center">
              <div className="flex items-center justify-center space-x-4">
                <div className="text-lg font-bold">🎭 MOVIES SPONSORLAR</div>
                <div className="text-sm opacity-90">Film İçerikleri Premium Reklam</div>
              </div>
              <div className="mt-2 text-xs opacity-75">Movies kategorisi reklam entegrasyonu alanı</div>
            </div>
            ...
            <div className="flex justify-center items-center space-x-4 mt-12 pt-8 border-t border-gray-200 md:border-gray-800">
              <span className="text-gray-600 md:text-gray-400 text-caption">Showing {movies.length} movies, Page 1</span>
          ...
          <aside className="hidden md:block w-72 bg-black rounded-xl p-6 h-fit shadow-professional-lg">
            <h2 className="text-white text-title mb-6">Movie Genres</h2>
            <div className="space-y-3">
              {["Action", "Adventure", "Comedy", "Drama", "Horror", "Sci-Fi", "Thriller", "Romance", "Fantasy", "Crime", "War", "Western", "Musical", "Biography", "Documentary"].map((genre) => (
                <a
                  key={genre}
                  href="#"
            ...
            {/* Featured Movies */}
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Top Movies</h3>
              <div className="space-y-4">
                {movies.slice(0, 4).map((movie) => (
                  <Link key={`featured-${movie.id}`} href={`/video/${movie.id}`} className="flex space-x-3 group cursor-pointer">
                    <img
                      src={movie.thumbnail}
                      alt={movie.title}
                    ...
                    <div className="flex-1">
                      <p className="text-white text-caption font-medium line-clamp-2 group-hover:text-gray-200 transition-colors">{movie.title}</p>
                      <p className="text-gray-400 text-label mt-1">{formatViews(movie.views)} views</p>
                    </div>
                  </Link>
                ))}
              </div>
            ...
            {/* Movie Archive Dropdown */}
            <div className="mt-10">
              <h3 className="text-white text-title mb-6">Release Year</h3>
